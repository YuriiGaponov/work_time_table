...
class Config:
    ...
    """
    Базовый URL для обращения к API календаря.

    Особенности:
    - Используется для формирования полных запросов к сервису календаря;
    - Значение задано как константа, так как endpoint API является
    фиксированным и не требует динамической конфигурации через
